...
/**
 * Returns a raw SVG string for the given emblem — usable in canvas compositing
 * without a DOM render. Mirrors the Emblem JSX below exactly.
 */
export function emblemSvgString(id: string, color: string, size = 100): string {
  const sw = 3
  const common = `fill="none" stroke="${color}" stroke-width="${sw}" stroke-linecap="round"`
  let inner = ''
  if (id === 'crest-rings') {
    inner = `<g ${common}>
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 100 100">
    <circle cx="50" cy="50" r="46" fill="#030509" stroke="${color}" stroke-width="1.5" opacity="0.9"/>
    ${inner}
  </svg>`
}
