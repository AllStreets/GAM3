...
0. **Balance/pacing still to tune** (from playtest, some fold into 7B): **reachability-aware contract generation** — offer/annotate contracts by whether some satellite can reach them within an affordable single burn (the "◀ best" bird can still be >800 m/s away on hard geography); **economy balance** — a full refuel (~§1080) can exceed a single reward (~§300–500); make the **completing pass cinematic** (7B) so the ~1–2 min wait after a burn pays off instead of feeling empty.
